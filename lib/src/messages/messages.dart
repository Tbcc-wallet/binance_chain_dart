...
class MultiTransferMsg extends Msg {
  ...
  MultiTransferMsg({required List<Transfer> transfers, String? memo, required Wallet wallet}) : super([wallet], memo) {
    _from_address = wallet.address;
    ...
    for (var transfer in transfers) {
      ...
      var currVal = coins[transfer.symbol];
      if (currVal != null) {
        coins[transfer.symbol] = currVal + transfer.amount_amino!;
      } else {
        coins[transfer.symbol] = transfer.amount_amino;
      }
    }
...
class NewOrderMsg extends Msg {
  ...
  NewOrderMsg({
    String? symbol,
    required TimeInForce time_in_force,
...
class VoteMsg extends Msg {
  ...
  late int _proposal_id;
  int? _proposal_id_encoded;
  late VoteOption _vote_option;
  String? _voter;
